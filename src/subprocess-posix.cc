...
#include "ninja_grpc.grpc.pb.h"

using grpc::Channel;
using grpc::ClientContext;
using grpc::Status;
using grpc::Status;
using ninja_grpc::Executor;
using ninja_grpc::Command;
using ninja_grpc::ExecuteResult;

class ExecutorClient {
 public:
  ExecutorClient(std::shared_ptr<Channel> channel)
      : stub_(Executor::NewStub(channel)) {}

  // 组装客户的有效载荷，发送它并呈现服务器的响应。
  std::string Execute(const std::string& cmd, const bool& console) {
    // Data we are sending to the server.
    Command request;
    request.set_str_command(cmd);
    request.set_use_console(console);
...
void work(string cmd, int fd_to_close, string target_str) {
  // 开线程，发任务
  ExecutorClient executor(
      grpc::CreateChannel(target_str, grpc::InsecureChannelCredentials()));
  bool use_console = false; //写死为false
  string reply = executor.Execute(cmd, use_console);
  // reply = "input_pip:" + reply + '\n';
  char* res_chs = strcpy((char*)malloc(reply.length() + 1), reply.c_str());
  write(fd_to_close, res_chs, strlen(res_chs));
  close(fd_to_close);
}
...
bool Subprocess::Start(SubprocessSet* set, const string& command) {
  ...
  thread t(work, command, output_pipe[1], target_str);
...
Subprocess *SubprocessSet::Add(const EdgeCommand& cmd) {
  ...
  if (!subprocess->Start(this, cmd.command)) {
    delete subprocess;
    return 0;
  }
